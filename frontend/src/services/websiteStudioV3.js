...
export const websiteStudioV3Api = {
  ...
  async publish(config) {
    const response = await api.post("/website-studio/publish", config);
    return response.data;
  },

  async unpublish() {
    const response = await api.post("/website-studio/unpublish");
    return response.data;
  },

  async getPublic(institutionCode) {
    const response = await api.get(
      `/website-studio/public/${encodeURIComponent(institutionCode)}`
    );
